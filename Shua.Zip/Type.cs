...
public readonly struct CompressionMethod(ushort value)
{
	...
	public bool IsStored => this.Value == 0;
	public bool IsDeflate => this.Value == 8;

	public static CompressionMethod FromUInt16(ushort value) => new(value);
}
...
public sealed class FileEntry
{
	...
	public static bool TryReadFromCentralDirectory(byte[] data, ref int position, out FileEntry? entry)
	{
		...
		CompressionMethod compressionMethod = CompressionMethod.FromUInt16(Binary.ReadUint16Le(data, ref position));
		...
		(ulong compressedSize, ulong uncompressedSize, ulong localHeaderOffset) = ParseZip64Extra(
			data,
			extraOffset,
			extraLength,
			compressedSize32,
...
public sealed class EndOfCentralDirectory
{
		...
		byte[] data)
	{
		...
		using Stream locatorStream = reader.OpenRead(locatorOffset, 20);
		...
		using Stream zip64Stream = reader.OpenRead((long)zip64EocdOffset, 56);
