...
internal sealed class ByteReader(byte[] data)
{
	...
	public int ReadInt()
	{
		int value = BitConverter.ToInt32(this._data, this._pos);
		this._pos += 4;
		return value;
	}
}
...
public sealed class CatalogParser
{
	...
	public CatalogParser(
		...
		byte[] entryData)
	{
		this._entries = Parse(keyData, bucketData, entryData).ToImmutableList();
	}

	public CatalogParser(Stream json)
	{
		Type.Catalog data = JsonSerializer.Deserialize(json, JsonContext.Default.Catalog) ??
	...
	private static List<CatalogEntry> Parse(
		...
		byte[] entryData)
	{
		ByteReader reader = new(bucketData);
		...
		for (int i = 0; i < bucketCount; i++)
		{
			...
			CatalogKeyType keyType = (CatalogKeyType)keyData[keyPos++];
			CatalogKey key;
